...
export default function MediaDetailScreen() {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dy > 100) togglePanel(false);
    ...
    if (lastTap.side === side && (now - lastTap.time) < DOUBLE_TAP_DELAY) {
      if (entry?.type === 'video') {
        ...
        if (s?.isLoaded) {
          ...
          const newPos = side === 'left' ? Math.max(0, s.positionMillis - seekAmount) : s.positionMillis + seekAmount;
  ...
  const toggleFullscreen = async () => {
      if (isFullScreen) await ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.PORTRAIT_UP);
  ...
  return (
    <View style={[styles.container, { backgroundColor: '#000' }]}>
      ...
      <Animated.View style={[
          ...
          { transform: [{ translateY: mediaTranslateY }] }
      ]}>
        {entry.type === 'video' ? (
            <View style={styles.playerWrapper}>
                <Video
                    ...
                    onPlaybackStatusUpdate={s => { if(s.isLoaded) setIsPlaying(s.isPlaying); setStatus(s); }}
                    isLooping={settings.loopVideos}
                    shouldPlay={settings.autoPlay}
                />

                {/* INTERACTION LAYER 1: Seek Regions */}
                <View style={styles.touchOverlay}>
                    <Pressable style={styles.touchThird} onPress={() => handleDoubleTapSeek('left')} />
                    <Pressable style={styles.touchThird} onPress={() => setShowControls(!showControls)} />
                    <Pressable style={styles.touchThird} onPress={() => handleDoubleTapSeek('right')} />
                </View>

                {/* INTERACTION LAYER 2: Play/Pause Button (Highest Layer) */}
                {showControls && (
        ...
        {showControls && (
            <View style={[styles.headerOverlay, isFullScreen && { top: 20 }]}>
                ...
                <View style={styles.headerRight}>
                    {!isFullScreen && <TouchableOpacity style={styles.circBtn} onPress={toggleFullscreen}><Maximize size={20} color="#FFF" /></TouchableOpacity>}
                    <TouchableOpacity style={styles.circBtn} onPress={() => { router.back(); setTimeout(() => toggleVault(entry.id), 100); }}>
        ...
        {showControls && !seekType && (
            <>
                {currentIndex > 0 && <TouchableOpacity style={[styles.navArrow, styles.leftArrow]} onPress={goToPrevious}><ChevronLeft size={40} color="white" /></TouchableOpacity>}
                {currentIndex < filteredSortedMedia.length - 1 && <TouchableOpacity style={[styles.navArrow, styles.rightArrow]} onPress={goToNext}><ChevronRight size={40} color="white" /></TouchableOpacity>}
            </>
        )}
      </Animated.View>
      ...
      {!isFullScreen && (
          <Animated.View
            style={[styles.detailsSheet, { backgroundColor: theme.background, transform: [{ translateY: panelTranslateY }] }]}
            {...panResponder.panHandlers}
          >
              <View style={styles.sheetHandleContainer}><View style={[styles.sheetHandle, { backgroundColor: theme.border }]} /></View>
              {!isPanelExpanded && <TouchableOpacity style={styles.expandTrigger} onPress={() => togglePanel(true)}><ChevronUp size={24} color={theme.primary} /><Text style={[styles.expandLabel, { color: theme.primary }]}>View Details</Text></TouchableOpacity>}
              <ScrollView style={styles.sheetContent} showsVerticalScrollIndicator={false}>
                  <View style={styles.sheetHeader}><Text style={[styles.mediaTitle, { color: theme.text }]}>{entry.title}</Text><TouchableOpacity onPress={() => togglePanel(false)}><ChevronDown size={24} color={theme.textMuted} /></TouchableOpacity></View>
                  <Text style={[styles.mediaDate, { color: theme.textMuted }]}>{formatDate(entry.media_date)}</Text>
                  <View style={styles.badgeRow}>
                      <View style={[styles.typeBadge, { backgroundColor: theme.surfaceElevated }]}><Tag size={14} color={theme.primary} /><Text style={[styles.badgeText, { color: theme.text }]}>{entry.type.toUpperCase()}</Text></View>
                      <View style={[styles.typeBadge, { backgroundColor: theme.surfaceElevated }]}><Calendar size={14} color={theme.secondary} /><Text style={[styles.badgeText, { color: theme.text }]}>SAVED</Text></View>
                  </View>
                  {entry.notes ? <View style={styles.memoSection}><Text style={[styles.memoHeader, { color: theme.textMuted }]}>MEMORY NOTES</Text><Text style={[styles.memoText, { color: theme.text }]}>{entry.notes}</Text></View> : null}
              </ScrollView>
          </Animated.View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  touchThird: { flex: 1 },
  playOverlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center', zIndex: 20 },
  ...
  leftArrow: { left: 5 },
  rightArrow: { right: 5 },
  detailsSheet: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 500, borderTopLeftRadius: 35, borderTopRightRadius: 35, elevation: 25 },
  sheetHandleContainer: { alignItems: 'center', paddingVertical: 15 },
  sheetHandle: { width: 50, height: 6, borderRadius: 3 },
  expandTrigger: { position: 'absolute', top: -45, alignSelf: 'center', paddingHorizontal: 20, height: 45, backgroundColor: 'rgba(255,255,255,0.95)', borderTopLeftRadius: 20, borderTopRightRadius: 20, flexDirection: 'row', alignItems: 'center', gap: 8, elevation: 5 },
